...
namespace LinearAlgebra
{
    ...
    public struct Matrix
    {
        ...
        public static int RandomChoice(Matrix m, System.Random r)
        {
            if(m.X != 1)
            {
                throw new ArgumentException("X dimension in matrix must be 1");
            }

            for(int i = 0 ; i < m.Y; i++)
            {
                if(r.NextDouble() < m[0, i])
                {
                    return i;
                }
            }
        ...
        public static Matrix DiagFlat(Matrix m)
        {
            if(m.Y != 1)
                throw new ArgumentException("The matrix must be flat");

            Matrix diagonal = Matrix.Zeros(m.X, m.X);
